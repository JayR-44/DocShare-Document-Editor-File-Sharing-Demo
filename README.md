...
- Delete an owned document after an explicit confirmation. Shared users cannot delete documents.
...
6. Switch back to Alex Morgan and delete an owned document. The confirmation prompt prevents accidental deletion.
...
See [`ARCHITECTURE.md`](./ARCHITECTURE.md) for the component boundaries, request flow, data model, and access rules. See [`AI-WORKFLOW.md`](./AI-WORKFLOW.md) for the AI-native development and verification note. See [`SUBMISSION.md`](./SUBMISSION.md) for the submission inventory.
...
Supabase Postgres stores `profiles`, `documents`, and `document_shares`. Formatted editor content is stored as JSON rather than HTML so its structure is preserved and it can safely be restored into TipTap. The relationship table makes sharing explicit and supports an Editor/Viewer permission model. Only owners can manage sharing or delete a document; deleting a document also removes its related shares through the database foreign-key cascade.
